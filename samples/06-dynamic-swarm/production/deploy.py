"""
Deploy Module 6: Dynamic Swarm — 1 AgentCore Runtime.

Pattern: Researcher → Analyst → Writer with autonomous handoffs (Strands Swarm).
All agents run inside ONE Runtime container.

Usage:
    python deploy.py
    python deploy.py --name-prefix m6ws
...
def main():
    parser = argparse.ArgumentParser(
        description="Deploy Module 6 Dynamic Swarm to AgentCore"
    )
    parser.add_argument("--name-prefix", default="m6",
                        help="Runtime name prefix, ≤20 chars (default: m6)")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    prefix = args.name_prefix[:20]
    runtime_name = f"{prefix}_swarm"
    ...
    role_name = f"agentcore-{prefix}-swarm-role"
