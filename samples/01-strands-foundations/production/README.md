# M1: Decision Intelligence Agent — Production Runtime
...
agentcore create          # name: 01-runtime (or any name)
cd <project-name>
...
```bash
# CLI
agentcore invoke "What is NovaCart's CLV and churn rate?"

# With session ID (for multi-turn context, must be 33+ chars):
agentcore invoke --session-id decision-memo-session-000001-abc "What is NovaCart's CLV and churn rate?"
```
...
response = client.invoke_agent_runtime(
    ...
    payload=json.dumps({
        "prompt": "What is NovaCart's CLV and churn rate?",
