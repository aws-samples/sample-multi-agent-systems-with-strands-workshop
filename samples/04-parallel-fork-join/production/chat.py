"""
Multi-turn chat for Module 4 Production: Parallel Fork-Join.

Each submission runs the full Researcher → [Analyzer A, B, C] → Synthesizer pipeline.
...
Requires:
  ...
  ANALYZER_RUNTIME_ARN    (from source .env_arns)
...
def main():
    print("Parallel Fork-Join Production | type 'quit' to exit")
    print(f"Researcher ARN : {os.environ.get('RESEARCHER_RUNTIME_ARN', 'NOT SET')[:60]}...")
    print(f"Analyzer ARN   : {os.environ.get('ANALYZER_RUNTIME_ARN', 'NOT SET')[:60]}...")
    print(f"Synthesizer ARN: {os.environ.get('SYNTHESIZER_RUNTIME_ARN', 'NOT SET')[:60]}...")
    print()
    ...
    while True:
        ...
        run_count += 1
        print(f"\n[Run {run_count}] Running Researcher → [Analyzer A, B, C] → Synthesizer...")
        print("─" * 60)
        try:
