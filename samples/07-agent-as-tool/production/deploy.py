"""
Deploy Module 7: Agent-as-Tool — 4 A2A specialists + 1 HTTP orchestrator.

Specialists (A2A, port 9000): research, finance, legal, writer
Orchestrator (HTTP, port 8080): LLM Agent that coordinates specialists as @tools

Use case: Investment analysis — orchestrator delegates to Research, Finance, Legal, Writer.
...
Usage: python deploy.py [--name-prefix m7] [--dry-run]
"""
import argparse, sys
...
def main():
    ...
    parser.add_argument("--name-prefix", default="m7", help="Runtime prefix, max 8 chars")
    parser.add_argument("--dry-run", action="store_true")
    args   = parser.parse_args()
    prefix = args.name_prefix[:8]

    specialists = {s: f"{prefix}_{s}" for s in ['research', 'finance', 'legal', 'writer']}
    ...
    specialist_defs = [
        (specialists["research"], HERE / "specialists/research"),
        (specialists["finance"],  HERE / "specialists/finance"),
        (specialists["legal"],    HERE / "specialists/legal"),
    ...
    orch_id, _ = u.create_runtime(ctl, orch_name, bucket, orch_key, orch_role, env_vars={
        "RESEARCH_RUNTIME_ARN": arns[specialists["research"]],
        "FINANCE_RUNTIME_ARN":  arns[specialists["finance"]],
        "LEGAL_RUNTIME_ARN":    arns[specialists["legal"]],
        "WRITER_RUNTIME_ARN":   arns[specialists["writer"]],
    })
