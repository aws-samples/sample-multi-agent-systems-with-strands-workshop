"""
Deploy Module 7: Agent-as-Tool — 4 AgentCore Runtimes.

Architecture:
  researcher  ─┐
  analyst     ─┤ (deployed in parallel, independent)
  synthesizer ─┘
...
SYNTHESIZER_RUNTIME_ARN from its environment at startup.

Usage:
    python deploy.py
    python deploy.py --name-prefix m7ws
...
def main():
    parser = argparse.ArgumentParser(description="Deploy Module 7 Agent-as-Tool to AgentCore")
    parser.add_argument("--name-prefix", default="m7",
                        help="Short prefix for runtime names, ≤8 chars (default: m7)")
    ...
    if args.dry_run:
        ...
        names = [f"{prefix}_researcher", f"{prefix}_analyst",
                 f"{prefix}_synthesizer", f"{prefix}_orchestrator"]
        for n in names:
    ...
    specialist_defs = [
        ...
        (analyst_name,     specialists_root / "analyst"),
        (synthesizer_name, specialists_root / "synthesizer"),
    ]
    ...
    orch_id, _ = u.create_runtime(
        ...
        env_vars={
            ...
            "ANALYZER_RUNTIME_ARN":    arns[analyst_name],
